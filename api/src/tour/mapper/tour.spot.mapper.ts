...
export class TourSpotMapper {
    static fromModelsToTourSpotResponses = (models: TourSpotDocument[]): TourSpotResponse[] => {
        return models.map(TourSpotMapper.fromModelToTourSpotResponse);
    };

    static fromModelToTourSpotResponse = (model: TourSpotDocument): TourSpotResponse => {
        return {
            id: model.id,
            name: model.name,
            audio: model.audio,
